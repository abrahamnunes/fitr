...
for t in range(ntrials):
    ...
    u  = rng.multinomial(1, pvals=fu.softmax(B1*q))
    x_, r, _ = env.step(u)
...
for t in range(R.size):
    ...
    #Dlp_lr1 += np.dot(u_, np.einsum('ij,jk,k->i', Dlp_q2, DQ_lr1, Dq2_Qmf))
    Dlp_lr2 += B1*np.einsum('i,ij,j->', u-pu1, (DQ_Qmb*DQmb_Qmf + DQ_Qmf)*DQmf_lr2, x)
    ...
    DQ_lr1 = DQ_Qmf*DQmf_lr1
    DQ_lr2 = DQ_Qmf*DQmf_lr2
