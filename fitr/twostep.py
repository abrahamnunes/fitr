...
class lr_cr_mf(object):
    ...
    cv_subject()
        Performs cross validation for a single subject
    cv_group()
        Performs cross validation for a group
    simulate()
    ...
    def cv_subject(self, params, states, actions, rewards):
        pass

    def cv_group(self, params, states, actions, rewards):
        pass

    def simulate(self, ntrials, nsubjects, preset_rpaths=None, rpath_max=0.75, rpath_min=0.25, rpath_sd=0.025, rpath_common=False):
        """
...
class lr_cr_et_mf(object):
    ...
    ----------
    LR : fitr.rlparams.LearningRate object
        Learning rate parameter object
    CR : fitr.rlparams.ChoiceRandomness object
        Choice randomness parameter object
    ET : fitr.rlparams.EligibilityTrace object
        Eligibility trace parameter object
    generative_model : str
        Stan code for fitr.MCMC
    ptrans : float in range (0, 1)
        The dominant transition probability (default = 0.7).
